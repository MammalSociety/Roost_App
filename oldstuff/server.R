...
shinyServer(function(input, output) {
  ...
  # this reactive output tells Shiny that once data has been uploaded it should 
  # display a download button for the Rmd report. If no data has been uploaded 
  #then the button will not appear.
  ...
  output$ui.download.helper <- renderUI({
    if (is.null(input$file)) return() #if no data uploaded, helper text associated
    #with the download button not present
  ...
  output$report <- downloadHandler(
    filename = "Ecobat Roost Report.doc", #we want word document output
    ...
    content = function(file) {
      ...
      #  ##sitename <- input$SiteName #tells Rmd what to use as Site Name
        ...
        params <- list(n = datab, m = datac) #Author = author, SiteName = sitename)
