...
#When working from Shiny app use these to load proforma and mastter, hash out when in RMD
...
For the purpose of this test report, the subset used is based on the data in the
test proforma. 
Species: `r proforma$Species`
Structure Type: `r proforma$StructureType`
Region: `r proforma$Region`
Month: March, April and May (not automatically pulled from proforma)
...
# Would this work if instead of putting 'House' put proforma$StructureType?
...
```{r}
# Percentiles for the subsetted data

srm <- structureregionmonth 
srm$percentiles <- ecdf(srm$TotalCount)(srm$TotalCount)*100

srm$percentileswhole <- lapply(srm$percentiles, as.integer)


Percentiles2 <- srm$TotalCount
Percentiles2.rank <- rank(Percentiles2, ties.method="min")
srm$percentiles2 <- round(Percentiles2.rank/length(Percentiles2.rank)*100)

sq0 <- quantile(srm$TotalCount, c(.0))
sq1 <- quantile(srm$TotalCount, c(.20))
sq2 <- quantile(srm$TotalCount, c(.40))
sq3 <- quantile(srm$TotalCount, c(.60))
sq4 <- quantile(srm$TotalCount, c(.80))

#Create percentile rank column
srm <- srm%>%
  mutate(
    percentilerank = case_when(
      percentiles2 >= sq0 & percentiles2 < sq1 ~ "low",
      percentiles2 >= sq1 & percentiles2 < sq2 ~ "low/moderate",
      percentiles2 >= sq2 & percentiles2 < sq3 ~ "moderate",
      percentiles2 >= sq3 & percentiles2 < sq4 ~ "moderate/high",
      percentiles2 >= sq4 ~ "high",
...
#Pull out percentile rank of proforma row from newmaster
srmproforma <- proforma

srmproforma$percentiles <- ecdf(srm$TotalCount)(srmproforma$TotalCount)*100
srmproforma$percentileswhole <- lapply(srmproforma$percentiles, as.integer)

Percentiles2 <- srmproforma$TotalCount
Percentiles2.rank <- rank(Percentiles2, ties.method="min")
srmproforma$percentiles2 <- round(Percentiles2.rank/length(Percentiles2.rank)*100)

srmproforma <- srmproforma%>%
  mutate(
    percentilerank = case_when(
      percentilessub >= q0 & percentilessub < q1 ~ "low",
      percentilessub >= q1 & percentilessub < q2 ~ "low/moderate",
      percentilessub >= q2 & percentilessub < q3 ~ "moderate",
      percentilessub >= q3 & percentilessub < q4 ~ "moderate/high",
      percentilessub >= q4 ~ "high",
...
#make column in proforma of percentile when using this subset
...
Your roost is the **`r srmproforma$percentileswhole`th percentile** when compared to the **`r unique(srm$refrange)`** bat roosts in the dataset. This means your roost is ranked as a **`r srmproforma$percentilerank`** number of bats.

```{r}

ggplot(structureregionmonth, aes(x="TotalCount", y=TotalCount))+geom_boxplot()+
  geom_point(
    data=proforma,
...
# Figure 5: 
A boxplot showing the proforma total count against other roots of the same species, structure type, region and season. 

```{r}

ggplot(srm, aes(x="percentiles", y=percentiles))+geom_boxplot()+
  geom_point(
    data=srmproforma,
...
#FINAL COMMAND OF REPORT NEEDS TO BE SAVING NEWMASTER AS THE MASTER
#CAREFUL WITH DOING THIS NOW BECAUSE THEN PROFORMA WILL BE DOUBLED WHEN RETESTING
#BECAUSE WILL BE SAVED IN MASTER.

#write.csv(newmaster, "./master.csv", row.names = FALSE)
...
### Appendix 1
This will explain why counts of 0 have not been included in the main report and will repeat some of the key data - maybe all of it, with 0s included. 




**Thank you for using the Ecobat Roost Analysis! If you have any questions please email info@themammalsociety.org**
